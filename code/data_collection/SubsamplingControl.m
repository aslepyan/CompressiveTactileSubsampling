...
numSensor = 1024;
samplingMode = [2]; % 1-downsampling; 2-random sampling; 3-binary sampling; 4-sum sampling !!
numSamplingMode = length(samplingMode);
save_time = 2; % duration of one window; s, !!
numWindow = 1; % num of window, !!
MNumFrameProd = 40e5; % threshold total num of msr sensors per window (2s is 2e5)
...
dataFolderPath = fullfile(uupwd, 'data', dataStorageFolder, dataFolderName);
dataFilePath = fullfile(uupwd, 'data', dataStorageFolder, dataFolderName, dataFileName);
